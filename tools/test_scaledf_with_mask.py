...
# -*- coding: utf-8 -*-
"""
ScaleDF Deepfake Detection 测试脚本 (Region-Aware Adapter, 带面部掩膜)

在 ScaleDF 独立测试集上评估 Region-Aware Adapter 模型性能
...
使用方法:
    python tools/test_scaledf_with_mask.py \
        --config configs/scaledf_region_adapter.yaml \
        --checkpoint checkpoints/scaledf_region_adapter/best_auc_model.pth
...
def load_model(
    ...
    """
    从 checkpoint 加载 Region-Aware Adapter 模型, 优先使用 EMA 权重.
    """
...
def plot_confusion_matrix(metrics: Dict, save_path: str):
    ...
    ax.set_title('Confusion Matrix (Region-Aware Adapter)', fontsize=15)
    plt.tight_layout()
...
def plot_roc_curve(labels: np.ndarray, probs: np.ndarray, auc_val: float, save_path: str):
    ...
    ax.set_title('ROC Curve (Region-Aware Adapter)', fontsize=15)
...
def print_results(metrics: Dict, stats: Dict):
    print(f"\n{'=' * 70}")
    print(f"ScaleDF 测试集评估结果 (Region-Aware Adapter)")
    print(f"{'=' * 70}")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='ScaleDF Deepfake Detection 测试脚本 (Region-Aware Adapter)',
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument('--config', type=str, default=None,
                        help='YAML 配置文件路径 (如 configs/scaledf_region_adapter.yaml)')
...
def main():
    ...
    # 打印实验信息
    print("=" * 70)
    print("ScaleDF Deepfake Detection - 测试脚本 (Region-Aware Adapter)")
    print("=" * 70)
    if cfg is not None:
    ...
    if args.no_ema:
        ...
        if isinstance(checkpoint, dict) and 'config' in checkpoint:
            saved_cfg = checkpoint['config']
            if isinstance(saved_cfg, dict):
                adapter_cfg = saved_cfg.get('model', {}).get('adapter', {})
                adapter_layers = adapter_cfg.get('layers')
                adapter_type = adapter_cfg.get('type', 'region_adapter')
                for key in ['bottleneck_dim', 'dropout', 'init_scale', 'scale_trainable']:
                    if key in adapter_cfg:
    ...
    result_data = {
        'experiment': cfg.experiment.name if cfg else 'scaledf_region_adapter',
