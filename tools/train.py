...
# -*- coding: utf-8 -*-
"""
Deepfake Detection 训练脚本 (CLIP ViT-L/14 + Adapter)

支持 YAML 配置文件控制实验参数

核心技术：
1. CLIP ViT-L/14 backbone (冻结) + 可学习适配器
2. 静态验证集：1:1 平衡采样，训练期间永不改变
3. EMA (Exponential Moving Average)：验证时使用 EMA 权重
4. Warmup + CosineAnnealing 学习率调度
5. Early Stopping：监控 EMA 模型的验证集 AUC

使用方法:
    # 使用 YAML 配置文件
    python tools/train.py --config configs/exp_cls_baseline.yaml
    
    # 命令行覆盖配置
    python tools/train.py --config configs/exp_cls_baseline.yaml --device 1 --batch_size 64
    
    # 启用 wandb
    python tools/train.py --config configs/exp_cls_baseline.yaml --use_wandb
"""
...
# 导入自定义模块
from lib.data.dataset import DF40Dataset
...
def init_wandb(cfg: Config, model, total_params: int, trainable_params: int):
    ...
    run = wandb.init(
        ...
        name=run_name,
        config=cfg.to_dict(),
        tags=['deepfake-detection', 'clip-vit', 'adapter'],
        reinit=True
    )
...
def train(cfg: Config, args):
    ...
    # 创建模型 (DF40 风格: 1024-dim CLS token + Linear classifier)
    ...
    print(f"\n模型: CLIP ViT-L/14 + Adapter (DF40 风格)")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Deepfake Detection Training with YAML Config',
