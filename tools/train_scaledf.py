...
# -*- coding: utf-8 -*-
"""
ScaleDF Deepfake Detection 训练脚本 (CLIP ViT-L/14 + Adapter)

基于 ScaleDF 数据集, 使用 YAML 配置文件控制实验参数

核心技术：
1. CLIP ViT-L/14 backbone (冻结) + 可学习适配器
2. ScaleDF 数据集: 按目录名区分真伪 (000000开头为真实)
3. 90/10 划分训练/验证集 (每个子文件夹内独立划分)
4. 自定义数据增强: Resize + JPEG压缩 + 50/25/25 随机扰动注入
5. EMA + Warmup + CosineAnnealing + Early Stopping

使用方法:
    python tools/train_scaledf.py --config configs/scaledf_baseline.yaml

    # 命令行覆盖配置
    python tools/train_scaledf.py --config configs/scaledf_baseline.yaml --device 1 --batch_size 64

    # 启用 wandb
    python tools/train_scaledf.py --config configs/scaledf_baseline.yaml --use_wandb
"""
...
from lib.data.scaledf_dataset import ScaleDFDataset
from lib.data.transfroms import get_scaledf_train_transforms, get_scaledf_val_transforms
...
def init_wandb(cfg: Config, model, total_params: int, trainable_params: int):
    ...
    run = wandb.init(
        ...
        name=run_name,
        config=cfg.to_dict(),
        tags=['deepfake-detection', 'clip-vit', 'adapter', 'scaledf'],
        reinit=True
    )
...
def train(cfg: Config, args):
    ...
    print("=" * 70)
    print("ScaleDF Deepfake Detection Training")
    print("=" * 70)
    ...
    train_transform = get_scaledf_train_transforms(config=aug_config)
    val_transform = get_scaledf_val_transforms(config=aug_config)
    ...
    print(f"  扰动概率: 50% 无 / 25% 1个 / 25% 2个")

    train_dataset = ScaleDFDataset(
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='ScaleDF Deepfake Detection Training with YAML Config',
