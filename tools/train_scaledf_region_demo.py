...
# -*- coding: utf-8 -*-
"""
ScaleDF Region-Decoupled MoE 训练脚本 (消融实验 3)

在消融实验 2 (Region-Aware Adapter) 基础上增加:
  - HDM (层次化解耦模块): 3 → 7 向量
  - CLS-Guided MoE: CLS token 引导的混合专家路由
  - BN + Linear → 真假二分类

核心技术：
  1. CLIP ViT-L/14 backbone (冻结) + RegionAdapter (可训练)
  2. 区域特征提取: 掩膜池化 → face/boundary/background 向量
  3. HDM: 3 向量 → 7 层次化特征向量 (learnable token + cross-attention)
  4. CLS-Guided MoE: CLS 全局信息引导 7 路由 → 融合特征
...
使用方法:
    ...
    python tools/train_scaledf_region_demo.py --config configs/scaledf_region_demo.yaml --device 1
    python tools/train_scaledf_region_demo.py --config configs/scaledf_region_demo.yaml --use_wandb
"""
...
def init_wandb(cfg: Config, model, total_params: int, trainable_params: int):
    ...
    run = wandb.init(
        ...
        name=run_name,
        config=cfg.to_dict(),
        tags=['deepfake-detection', 'clip-vit', 'region-demo', 'hdm', 'moe', 'scaledf'],
        reinit=True
    )
...
def train(cfg: Config, args):
    ...
    # HDM 配置
    hdm_cfg = cfg.model.get('hdm', None)
    hdm_num_heads = getattr(hdm_cfg, 'num_heads', 16) if hdm_cfg else 16
    hdm_dropout = getattr(hdm_cfg, 'dropout', 0.1) if hdm_cfg else 0.1

    # MoE 配置
    moe_cfg = cfg.model.get('moe', None)
    moe_num_experts = getattr(moe_cfg, 'num_experts', 7) if moe_cfg else 7
    moe_num_heads = getattr(moe_cfg, 'num_heads', 16) if moe_cfg else 16

    # 特征维度
    feat_dim = getattr(cfg.model, 'feat_dim', 1024)
    grid_size = getattr(cfg.model, 'grid_size', 16)

    print("=" * 70)
    print("ScaleDF Region-Decoupled MoE Training (消融实验 3)")
    print("=" * 70)
    ...
    print(f"\n训练集: {train_stats['real']} 真实 + {train_stats['fake']} 伪造 = {train_stats['total']} 样本")
    print(f"验证集: {val_stats['real']} 真实 + {val_stats['fake']} 伪造 = {val_stats['total']} 样本")
    ...
    print("\n" + "=" * 70)
    print("训练完成!")
    print(f"最佳 AUC 模型: Epoch {best_auc_epoch}, AUC: {best_auc:.4f}")
    print(f"  保存位置: {save_dir / 'best_auc_model.pth'}")
    print(f"最佳 ACC 模型: Epoch {best_acc_epoch}, ACC: {best_acc*100:.2f}%")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='ScaleDF Region-Decoupled MoE Training (消融实验 3)',
